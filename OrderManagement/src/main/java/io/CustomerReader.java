...
public class CustomerReader {
    ...
    CustomerDAOImplements function = new CustomerDAOImplements();
    ArrayList<Customer> tmpCustomer = function.customer;

    public boolean checkDuplicated(String id) {
        Boolean check = false;
        for (int index = 0; index < tmpCustomer.size(); index++) {
            if (id.equals(tmpCustomer.get(index).getCustomerID())) {
                check = true;
            }
        }
...
//Add customer
    ...
    public Customer inputCustomer() {
        ...
        while (tmp == null) {
            ...
            if (checkID(customerID) && checkName(customerName) && checkAddress(customerAddress) && checkPhone(customerPhone)) {
